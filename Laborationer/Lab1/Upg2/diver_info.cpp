...
//	in.getline(j.name, NAME_LENGTH);  // frŒn in till structen diver
	if (!(in.getline(j.name, NAME_LENGTH))){ // om inte namn ges fel
		j.difficulty=ERROR;
		return in;
	}
...
bool read_line(istream& in, double d[], double &dif){
	...
	double score; 
	int i = 0;
	while (streamed >> score && (i < NR_OF_REF)) { // sŒlŠnge det Šr en double
		d[i++] = score;
	}
	streamed >> ws; // tar bort whitespace
	if(streamed.eof() && i == NR_OF_REF){ // if last bite return true
		return true;
	}
	else{
		return false;
