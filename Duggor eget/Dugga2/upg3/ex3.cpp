...
class Employee {
  public:
    ...
    friend ostream& operator<<(ostream& os,Employee &X);
...
std::ostream& operator<<(ostream& os, Employee &X){
    X.display(os);
    return os;
};
...
//constructor
Teacher::Teacher(string s, string *c, int n) : Employee(s) //call constructor of base class (needs to be inisilized before)
{
    n_courses = n;
